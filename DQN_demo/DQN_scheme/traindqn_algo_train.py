...
import numpy as np
import Algorithm.Dqn.project_backend as pb
import time
import collections
import json
import Algorithm.Dqn.DQN as DQN
import argparse
...
def main(args):
    ...
    for overal_sims in range(simulation,simulation+num_simulations):
        ...
        file_path = './Channel_generator/Channel/%s_network%d' % (args.json_file, overal_sims)
        ...
        #    strategy_translation_dB_step = Pmax_dB/(policy.num_actions-1)
        #    for i in range(1,policy.num_actions-1):
        #        strategy_translation[i] = i *(Pmax/(policy.num_actions-1))
        ...
        #    for i in range(2,policy.num_actions-1):
        #        strategy_translation[i] = Pmin * ((Pmax/Pmin) ** ((i-1.0)/(policy.num_actions-2)))
        #    strategy_translation[-1] = Pmax
        ...
        with tf.Session() as sess:
            ...
            for sim in range (total_samples):
                ...
                Dqn_sinr_interference.append(pb.sinr_interference_list(H_all[sim], p_strategy, N, noise_var, weights[sim]))
                p_strategy_all.append(np.array(p_strategy))
                if(sim%2500 == 0):
                    print('Time %d sim %d'%(sim,overal_sims))
                if (sim % 500 == 0):
                    print("| Oversim: ", overal_sims, "|")
                    print('| Sim: {:d} | Sum-rate: {:f} |'.format(sim, sum_rate_distributed_policy[-1]))
                    print("| Power: ", p_strategy_all[-1], "|")

            policy.equalize(sess)
            print('Train is over')
    
            model_destination = ('./Proposed_scheme/Trained_model/Dqn/%s.ckpt'%(args.json_file))
            policy.save(sess,model_destination)
               
        # End Train Phase
        np_save_path = './Simulations/Sumrate/Train/Dqn/%s_%s_network%d.ckpt' % (args.json_file, args.json_file_policy, overal_sims)
        print(np_save_path)
        np.savez(np_save_path,options,options_policy,sum_rate_distributed_policy,p_strategy_all,Dqn_sinr_interference,
...
if __name__ == "__main__": 
    ...
    parser.add_argument('--json-file', type=str, default='K7_N7/train_K7_N7_shadow8_episode1-10000_travel0_vmax0_0',
