...
def sinr_interference_list(h, p, n, noise_var, weights):
    H_2 = (h**2)
    sinr = []
    interference = []
    for loop in range(n):
        tmp_1 = H_2[loop, loop] * p[loop]
        tmp_2 = np.matmul(H_2[loop, :], p) + noise_var - tmp_1
        interference.append(tmp_2)
        if(tmp_1/tmp_2 > treshold_sinr):
            sinr.append(np.log2(1.0+treshold_sinr))
        else:
            sinr.append(np.log2(1.0+tmp_1/tmp_2))
    return sinr,interference
