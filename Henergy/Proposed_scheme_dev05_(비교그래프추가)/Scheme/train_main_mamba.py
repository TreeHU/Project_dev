...
def parse_args() -> Config:
    ...
    p.add_argument("--data_path", type=str, default="./Project/Henergy/Proposed_scheme_dev05_(비교그래프추가)/Data_generator/Data_output/generated_1hour_data_pv_ghi_clearghi_output_y.xlsx",
...
def main():
    ...
    log_csv = os.path.join(log_dir, f"train_log_mamba_{now_korea.strftime('%Y%m%d_%H%M%S')}.csv")
    logs = []
    ...
    # 모델 저장
    agent.save("vpp_actorcritic_mamba.pt", obs_dim=env.obs_dim)
    print("모델이 저장되었습니다: vpp_actorcritic_mamba.pt")
