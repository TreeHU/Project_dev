...
class RewardCallback(BaseCallback):
    def __init__(self, window_size: int = 10_000):
        ...
        # [NEW] 10k step 단위 reward 합 기록
        self.window_size = window_size
        self._window_sum = 0.0
        self.train_window_reward_sum = []   # 각 윈도우(10k) 총합 reward
        self.train_window_end_step = []     # 해당 윈도우 끝 step index(1-based)
    ...
    def _on_step(self) -> bool:
        ...
        # [NEW] 10k-window reward 합
        self._window_sum += reward
        step_count = self.num_timesteps  # SB3가 누적 관리하는 training step 수
        if step_count % self.window_size == 0:
            self.train_window_reward_sum.append(self._window_sum)
            self.train_window_end_step.append(step_count)
            self._window_sum = 0.0
...
if __name__ == "__main__":
    ...
    #  B) 학습
    # ==========================
    #model.learn(total_timesteps=200_000, progress_bar=True, callback=reward_callback) #기존 코드 주석 KH
    #model.learn(total_timesteps=1000_000, progress_bar=True, callback=reward_callback)

    #model.learn(total_timesteps=200_000, progress_bar=True, callback=reward_callback) #Basic_PPO_episode_5_HEGY_SHARE_0.01->0.20
    ...
    fig_path6 = os.path.join(fig_dir, f"Train - smp_krw_per_kwh_{now_korea.strftime('%Y%m%d_%H%M%S')}.png")
    fig_path7 = os.path.join(fig_dir, f"Train - RewardSum per 10k steps_{now_korea.strftime('%Y%m%d_%H%M%S')}.png"
)
    ...
    plt.savefig(fig_path4, dpi=150)
     ...
     # (선택) 너무 길면 1/N 샘플링해서 그리기
    stride = 10  # 10이면 10step마다 하나만 표시(필요시 50/100으로)
    dispatch_series = np.array(reward_callback.train_dispatch, dtype=float)[::stride]
    smp_series = np.array(reward_callback.train_smp, dtype=float)[::stride]

    # 7) [NEW] 학습 중 10,000 step 단위 총합 reward
    plt.figure(figsize=(12,4))
    plt.plot(reward_callback.train_window_end_step, reward_callback.train_window_reward_sum)
    plt.title("Training - Total Reward per 10,000 Steps")
    plt.xlabel("Training Step (window end)")
    plt.ylabel("Sum of Reward in Window")
    plt.grid(True)
