...
class RewardCallback(BaseCallback):
    def __init__(self):
        ...
        self.train_smp = []
        self.train_dispatch = []
        self.train_datetime = []  # 있으면 좋음(없으면 step index로 그림)
    ...
    def _on_step(self) -> bool:
        ...
        # [NEW] info에서 smp/dispatch/datetime 기록
...
if __name__ == "__main__":
    ...
    # 6) 학습 중 smp_krw_per_kwh 시계열
    plt.figure(figsize=(12,4))
    plt.plot(smp_series)
    plt.title("Training - smp_krw_per_kwh (sampled)")
    plt.xlabel(f"Step (every {stride} steps)")
